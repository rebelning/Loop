...
class LoopLogger {
    ...
    init(category: String) {
        logger = Logger(subsystem: "com.randallknutson.loop", category: category)
    }
    ...
    public func hexInfo(_ msg: String,hexData: Data, file: String = #file, _ function: String = #function, _ line: Int = #line) {
        let hexString = hexData.map { String(format: "%02X", $0) }.joined()
        let message = "\(file.file) - \(function)#\(line): \(msg) = \(hexString)"
        self.logger.info("\(message, privacy: .public)")
        self.writeToFile(message, .info)
    }
    ...
    private var logFile: String {
        getDocumentsDirectory().appendingPathComponent("loop/log.txt").path
    }

    private var logDir: String {
        getDocumentsDirectory().appendingPathComponent("loop").path
    }

    private var logFilePrev: String {
        getDocumentsDirectory().appendingPathComponent("loop/log_prev.txt").path
    }
